...
export class AIClient {
  private static async callGeminiAPI(prompt: string, modelId: string): Promise<string> {
  ...
  private static async callOpenRouterAPI(prompt: string, modelId: string, isCodeGeneration = false): Promise<string> {
    ...
    const messages = isCodeGeneration 
      ? [
          {
            role: 'system',
    ...
    const response = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      headers: {
        ...
        'HTTP-Referer': process.env.NEXT_PUBLIC_APP_URL || 'https://bolt-new.netlify.app',
  ...
  static async chat(prompt: string, modelId: string): Promise<ChatResponse> {
    try {
      ...
      if (selectedModel.provider === 'gemini') {
        response = await this.callGeminiAPI(prompt, modelId);
      } else if (selectedModel.provider === 'openrouter') {
        response = await this.callOpenRouterAPI(prompt, modelId);
      } else {
      ...
      return { 
        result: "I'm sorry, I encountered an error. Please check your API keys and try again.",
        error: error.message 
      };
    }
  }

  static async generateCode(prompt: string, modelId: string): Promise<CodeResponse> {
    try {
      const selectedModel = AI_MODELS.find(m => m.id === modelId) || AI_MODELS[0];
      ...
      if (selectedModel.provider === 'gemini') {
        // For Gemini, we need to use the code generation session
        const { GoogleGenerativeAI } = await import("@google/generative-ai");
        ...
        response = await this.callOpenRouterAPI(prompt, modelId, true);
      } else {
        throw new Error(`Unsupported model provider: ${selectedModel.provider}`);
      }
      ...
      return { 
        projectTitle: "Error",
        explanation: "I'm sorry, I encountered an error while generating code. Please check your API keys and try again.",
        files: {},
        generatedFiles: [],
        error: error.message 
      };
